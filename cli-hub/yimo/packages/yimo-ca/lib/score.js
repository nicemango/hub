const defaultScorePlugin = (analysisContext) => {
  const { pluginsQueue, browserQueue, importItemMap } = analysisContext;

  const mapNames = pluginsQueue
    .map((item) => item.mapName)
    .concat(browserQueue.map((item) => item.mapName));
  ...
  if (mapNames.length > 0) {
    mapNames.forEach((item) => {
      Object.keys(analysisContext[item]).forEach((element) => {
        if (analysisContext[item][element].isBlack) {
          score = score - 5;
          message.push(`${element} 属于黑名单api，请勿使用`);
        }
      });
    });
  }
